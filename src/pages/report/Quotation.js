...
function Quotation() {
    ...
    const getCodeProduct = () => {
        let data = [];
        objParam?.data_product.forEach((val, index) => {
            (data[val?.product?.name] = data[val?.product?.name] || []).push({
                index: index,
            });
        }, {});
        ...
        Object.keys(data).forEach((val, index) => {
            data[val].forEach((value, index_value) => {
                result[value.index] = {
                    code: val.substr(0, 1) + (index_value + 1),
                };
            });
        });
            ...
            <div className="flex justify-center py-10 print:mx-[1rem]">
                <div className="relative container rounded print:border-0 print:p-0 print:max-w-full">
                    <div className="absolute top-0 left-0 print:top-0 print:left-0">
                        <img src={LGlogo1} className="w-[280px]" alt="Logo Handex" />
                    </div>
                                ...
                                <table className="text-[14px] mt-2">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td className="w-[24rem]"> : {objParam?.data_form?.alamat}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                {objParam?.data_product.map((val, index) =>
                                    val.model_product.value.map((val_model, index_model) =>
                                        <tr key={index_model + index}>
                                            ...
                                            <td className="border px-2 border-slate-300 text-center">{getCodeProduct()[index]?.code}</td>
                                ...
                                <tbody>
                                    {objParam?.data_product.map((val, index) =>
                                        <tr key={index}>
                                            ...
                                            <td className="border px-2 border-slate-300 text-left">{getCodeProduct()[index]?.code} - {val.product_series.value.name} - {val.product_model.value.name}</td>
                    ...
                    <div className="print-break">
                        <div className="mt-5">
                            <span className="font-medium">Demikian surat penawaran ini kami ajukan. Besar harapan kami dapat bekerja sama dengan Bpk./Ibu.
                                <br></br>Atas perhatiannya kami ucapkan terima kasih.
                            </span>
                        </div>
                        <div className="mt-3">
                            ...
                            <ul className="font-medium">
                                ...
                                <li>- Penawaran Harga ini berlaku 7 (tujuh) hari dari tanggal Penawaran Harga.</li>
                        ...
                        <div className="flex justify-between mt-[2rem] mx-20">
                            <table className="text-center font-medium">
                                <tbody>
                                    <tr><td>Diterima,</td></tr>
                                    <tr><td> </td></tr>
                                    <tr><td className="pt-24">(<span className="mx-4">{objParam?.data_form?.jabatan_contact}</span>)</td></tr>
                                </tbody>
                            </table>
                            <table className="text-center font-medium">
                                <tbody>
